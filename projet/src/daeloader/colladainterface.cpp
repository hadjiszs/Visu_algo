...
void ColladaInterface::readGeometries(std::vector<ColGeom>* v, const char* filename) {
     ...
     int prim_count(0), num_indices(0);
...
//                    std::cout << "\n\n\n\v-------------!!--------------]]]]]]]]]]]\n\n\v" << std::endl;
                    ...
                    ((float*)source_data.data)[0] = strtod(strtok(text, " "), nullptr);
                    for(unsigned int index=1; index<num_vals; index++) {
                         tmp = strtok(NULL, " ");
                         ((float*)source_data.data)[index] = strtod(tmp.c_str(), nullptr);
//                         std::cout << "[" << tmp << "::" << ((float*)source_data.data)[index] << "]";
                    }
                    break;
               ...
               case 1:
                    ...
                    // Read the int values
                    ((int*)source_data.data)[0] = strtod(strtok(text, " "), nullptr);
                    for(unsigned int index=1; index<num_vals; index++) {
                         ((int*)source_data.data)[index] = strtod(strtok(NULL, " "), nullptr);
                    }
                    break;
