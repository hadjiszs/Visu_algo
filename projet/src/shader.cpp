...
Shader::Shader(std::string ver_shad, std::string fra_shad, bool gestion_text)
{
     std::string	buffer;
...
void Shader::charger(QOpenGLFunctions* function_contexte)
{
     ...
     _resolve_fonctions->glBindAttribLocation(_id_shader_program, 0, "in_position");
     if(_gestion_text)
          _resolve_fonctions->glBindAttribLocation(_id_shader_program, 1, "in_uv_map");
     else
          _resolve_fonctions->glBindAttribLocation(_id_shader_program, 1, "in_normals");
     ...
     if(_gestion_text)
          _id_texture  = _resolve_fonctions->glGetAttribLocation(_id_shader_program, "in_uv_map");
     else
          _id_normal   = _resolve_fonctions->glGetAttribLocation(_id_shader_program, "in_normals");
}
...
void Shader::del()
{
     ...
     _resolve_fonctions->glDisableVertexAttribArray(_id_normal);
}
...
void Shader::set_id_normal(GLint id)
{ _id_normal = id; }
