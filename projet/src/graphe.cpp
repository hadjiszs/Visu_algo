 ...
 */
Graphe::Graphe(std::string path, TextBox* textbox)
     : Structure(path, textbox)
{
...
void Graphe::charger()
{
     ...
     // Initialisation des etats de chaque noeud
     _vertex_to_etat.resize(_vertices.size());
     for(unsigned int i = 0; i < _vertices.size(); i++)
          _vertex_to_etat[i] = Etat::Noeud::SIMPLE;

     _vertex_to_etat[0] = Etat::Noeud::COURANT;

     _est_init = true;
}

inline void Graphe::compute_scale()
{
     // Norme entre deux points dans R^3
     auto norm =
     ...
     // Recuperation de la norme la plus petite
     float min_norme;
     float curr_norme;

     min_norme = norm(_vertices[0].Position[0], _vertices[0].Position[1], _vertices[0].Position[2],
                      _vertices[1].Position[0], _vertices[1].Position[1], _vertices[1].Position[2]);

     for(unsigned int i = 2; i < _vertices.size()-1; i+=2)
     {
          curr_norme = norm(_vertices[i+0].Position[0], _vertices[i+0].Position[1], _vertices[i+0].Position[2],
                            _vertices[i+1].Position[0], _vertices[i+1].Position[1], _vertices[i+1].Position[2]);

          if(min_norme > curr_norme)
               min_norme = curr_norme;
     }

     std::cout << "\n[Plus petite norme] -> [" << min_norme << "]" << std::endl;
     _scale = min_norme/3;
...
void Graphe::DFS_visiter_noeud(sommet_t* u, int* time, parcours_t* p)
{
     ...
     auto afficher =
          [=](int debut = -1, int fin = -1)
          {
               ...
               if(debut != -1)
               {
                    sprintf(buff, "[%2d] {D: %2d ; F: __ }", num_ligne, debut);
                    _text_box->ajouter(buff, num_ligne);
               }
               else if(fin != -1)
               {
                    sprintf(buff, " F: %2d }", fin);
                    _text_box->remplacer(buff, num_ligne, 13);
               }
          };
     ...
     *time += 1;
     ...
     _vertex_to_etat[u->val] = Etat::Noeud::COURANT;
     ...
     u->couleur = NOIR;
     std::this_thread::sleep_for(std::chrono::seconds(1));
     _vertex_to_etat[u->val] = Etat::Noeud::VISITED;
     *time += 1;
     u->f = *time;
     afficher(-1, u->f);
}
